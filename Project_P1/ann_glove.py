...
# add simple conversation context (Just in case also)
...
# GloVe embeddings
# You can swap to: "glove-wiki-gigaword-100" for faster/smaller
GLOVE_NAME = "glove-wiki-gigaword-300"

print(f"Loading GloVe model: {GLOVE_NAME} (first time downloads once)")
glove = api.load(GLOVE_NAME)   # downloads if needed
EMB_DIM = glove.vector_size

_token_re = re.compile(r"[A-Za-z']+")

def text_to_glove_vec(text: str) -> np.ndarray:
...
def encode_glove(texts):
    ...
    for i, t in enumerate(tqdm(texts, desc="GloVe encoding")):
        X[i] = text_to_glove_vec(t)
    return X

X_train = encode_glove(train_texts)
X_dev   = encode_glove(dev_texts)
X_test  = encode_glove(test_texts)
...
model = MLP(in_dim=X_train.shape[1]).to("cpu")
...
out.to_csv("outputs/predictions_ann_glove.csv", index=False)
print("\nSaved:", "outputs/predictions_ann_glove.csv")